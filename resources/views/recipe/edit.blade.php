    ...
    <div class="new-recipes m-t">
        <form action="{{ route('recipes_edit', ['id' => $recipe->id]) }}" method="post" enctype="multipart/form-data">
            ...
            <fieldset class="form-input">
                ...
                <input type="text" name="mini_description" placeholder="Мини описание (превью)" value="{{ $recipe->mini_description }}" required>
                @error('mini_description')
                <div><p>{{ $message }}</p></div>
                @enderror

                <label>Описание:</label>
                <input type="text" name="description" placeholder="Описание" required value="{{ $recipe->data['description'] ?? 'Описание отсутствует' }}">
                @error('description')
                <div><p>{{ $message }}</p></div>
                @enderror
            </fieldset>
            ...
            <fieldset class="form-input grid">
                ...
                <div class="grid">
                    ...
                    <input type="number" name="calorie" min="0" placeholder="Введите кол-во калорий" value="{{ $recipe->data['calorie'] ?? 'Калорийность не указана' }}" required>
                    @error('calorie')
                    <div><p>{{ $message }}</p></div>
                    @enderror
                </div>
                ...
                <div id="ingredients-container-edit">
                    @foreach($recipe->data['ingredients'] as $ingredient)
                        <div class="ingredient">
                            <input type="text" name="ingredients[]" value="{{ $ingredient['name'] }}" placeholder="Ингредиент">
                            <input type="number" name="ingredient_quantity[]" value="{{ $ingredient['quantity'] ?? '' }}" placeholder="Количество">
                            <select name="ingredient_unit[]">
                                <option value="г" @if(isset($ingredient['unit']) && $ingredient['unit'] == 'г') selected @endif>г</option>
                                <option value="кг" @if(isset($ingredient['unit']) && $ingredient['unit'] == 'кг') selected @endif>кг</option>
                                <option value="мл" @if(isset($ingredient['unit']) && $ingredient['unit'] == 'мл') selected @endif>мл</option>
                                <option value="л" @if(isset($ingredient['unit']) && $ingredient['unit'] == 'л') selected @endif>л</option>
                                <option value="шт" @if(isset($ingredient['unit']) && $ingredient['unit'] == 'шт') selected @endif>шт</option>
                                <option value="чашка" @if(isset($ingredient['unit']) && $ingredient['unit'] == 'чашка') selected @endif>чашка</option>
